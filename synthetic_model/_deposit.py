...
def getHeight(inA):
    ...
    for i3 in numba.prange(inA.shape[0]):
        for i2 in range(inA.shape[1]):
            height[i3, i2] = 0
            while (
                inA[i3, i2, height[i3, i2]] == -1 and height[i3, i2] < inA.shape[2] - 1
            ):
                height[i3, i2] += 1
    ...
    for i3 in numba.prange(array.shape[0]):
        for i2 in range(array.shape[1]):
            ...
            for i1 in range(height[i3, i2]):
                v = d * i1
                i = int(v)
                isinc = min(sinc.shape[0] - 1, int((v - i) * 10000 + 0.5))
                array[i3, i2, i1] = 0
...
class Deposit(Event):
    ...
    def applyBase(
        ...
        inM: GeoModel,
        thick: int = 30,
        depthGrad: float = 30.0,
        interbedThickAvg: float = 7.5,
        interbedThickDev: float = 0.0,
        interbedPropDev: float = 0.0,
        prop: float = 2500.0,
        expand: float = True,
        noiseFreq: float = 1,
        noiseLacunarity: float = 2.0,
        noisePersistence: float = 0.5,
        noiseAbs: bool = False,
        noiseZStretch=1.0,
        interbedOctaves=3,
        interbedDev=0.0,
        interbedFreq=2.0,
        interbedLacunarity=2.0,
        interbedPersistence=0.5,
        interbedZStretch=1.0,
        seed=None,
        noiseOctaves: int = 3,
        noiseDev: float = 0.0,
        layer=None,
        **kw,
        ...
        if layer == None:
            layer1D, nlayer = calc1DDeposition(
                n1Use,
                np.random.normal(
                    interbedThickAvg / axes[0].d, interbedThickDev / axes[0].d, (n1Use,)
                ),
            )
        ...
        height = getHeight(outM.getLayer().get_nd_array())

        z = np.linspace(
            0,
            3.0 * n1Use * axes[0].d / maxE * noiseZStretch * 4,
            4 * n1Use,
            dtype=np.float32,
        )
        y = np.linspace(0, 3.0 * exts[1] / maxE, axes[1].n, dtype=np.float32)
        x = np.linspace(0.0, 3.0 * exts[2] / maxE, axes[2].n, dtype=np.float32)
        zL = np.linspace(
            0,
            3.0 * n1Use * axes[0].d / maxE * interbedZStretch,
            n1Use,
            dtype=np.float32,
        )

        layerNG = SimplexNoise(random.randint(0, 1000 * 1000 * 1000))
        noiseG = SimplexNoise(random.randint(0, 1000 * 1000 * 1000))
        noiseV = noiseG.noise3D(
            z,
            y,
            x,
            frequency=noiseFreq,
            lucanarity=noiseLacunarity,
        ...
        noise3D = layerNG.noise3D(
            zL, y, x, n_octaves=interbedOctaves, amplitude=interbedDev
        )
        ...
        for fld in outM.getFloatFieldList():
            ...
            if fld == mainProp:
                ...
                if method != 0:
                    z = np.linspace(
                        0,
                        3.0 * n1Use * axes[0].d / maxE * kProp["noiseZStretch"],
                        n1Use,
                        dtype=np.float32,
                    )
                    propV = noiseG.noise3D(
                        z,
                        y,
                        x,
                        n_octaves=kProp["noiseOctaves"],
                        frequency=kProp["noiseFreq"],
                        lucanarity=kProp["noiseLacunarity"],
                        persistence=kProp["noisePersistence"],
                    ...
                    if method == 1:
                        ...
                        if kProp["layer"] == None:
                            lVal1D = np.random.normal(
                                kProp["prop"], kProp["interbedPropDev"], (n1Use,)
                            )
                        else:
                            if not isinstance(kProp["layer"], np.ndArray):
                                raise Exception("layer must be a np.ndarray")
                            loc1D = layer.get_nd_array()
                            lVal1D = np.linspace(0, loc1D.shape[0] - 1, loc1D.shape[0])
                            nlayerD = loc1D.shape[0]
                            if loc1D.shape[0] >= n1Use:
                                raise Exception(
                                    "layer is not large enough must be at least %d",
                                    n1Use,
                                )
                        noiseT = createPropArray(
                            nlayer,
                            kProp["depthGrad"] / axes[0].d,
                            noise3D,
                            layer1D,
                            lVal1D,
                            propV,
        ...
        layer = outM.getLayer().get_nd_array()

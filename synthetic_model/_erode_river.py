...
class ErodeRiver(Event):
    ...
    def applyBase(
        ...
        for fld in outM.getFloatFieldList():
            if fld != "indicator" and fld != "rgt":
                if isinstance(fill_prop, dict):
                    ...
                    fuse = fill_prop
            putRiver(
                path, outM.getFloatField(fld).get_nd_array(), fuse, thick / axes[0].d
            )
        if indicateF:
            putRiver(
                path,
                outM.getCreateFloatField("indicator", 0, 0).get_nd_array(),
                indicateValue,
                thick / axes[0].d,
            )
        ...
        if indicateI:
            putRiver(
                path,
                outM.getCreateIntField("indicator", 0, 0).get_nd_array(),
                indicateValue,
                thick / axes[0].d,
            )

        putRiver(path, outM.getLayer().get_nd_array(), inext, thick / axes[0].d)

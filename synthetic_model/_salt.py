...
"""
import numpy as np
from synthetic_model._model_noise import fractal
        ...
        for i2 in prange(top.shape[1]):
            if i2 >= b2_1 and i2 < e2_1 and i3 >= b3_1 and i3 < e3_1:
                ...
                if i3 >= e3_1 and i3 < e3_0:
                    sc3 = 1.0 - float(i3 - e3_1) / float(e3_0 - e3_1)
                if i2 >= b2_0 and i2 < b2_1:
                    sc2 = float(i2 - b2_0) / float(b2_1 - b2_0)
                if i2 >= e2_1 and i2 < e2_0:
                    sc2 = 1.0 - float(i2 - e2_1) / float(e2_0 - e2_1)
                dst = math.sqrt(sc2 * sc2 + sc3 * sc3)

                top[i3, i2] = (1.0 - dst) * topE + dst * top[i3, i2]
                bot[i3, i2] = (1.0 - dst) * botE + dst * bot[i3, i2]
...
def find_2d_noise_field(n1, n2, d=1.0, baseFreq=3.0, noise_levels=4, normalize=True):
...
@njit(parallel=True)
def createHillMap(
    salt: np.ndarray, up: np.ndarray, topE: np.ndarray, botE: np.ndarray, maxS: float
):
    ...
    for i3 in prange(fld.shape[0]):
        for i2 in prange(fld.shape[1]):
            for i1 in range(fld.shape[2]):
                fld[i3, i2, i1] = fld[i3, i2, int(0.5 + mapH[i3, i2, i1])]
...
class Salt(Event):
    ...
    def applyBase(
        ...
        ns = inM.getPrimaryProperty().get_hyper().get_ns()

        if method_2d:
            slt, top, bot = create_3d(
                ns[2],
                ns[1],
                ns[0],
                seed=seed,
                salt_value=1,
                thickness=thickness,
                n_octaves_top=n_octaves_top,
                n_octaves_bot=n_octaves_bot,
                amplitude_top=amplitude_top,
                amplitude_bot=amplitude_bot,
                beg2=beg2,
                beg3=beg3,
                end2=end2,
                end3=end3,
            ...
            if applyHills:
                shift = slt.copy()
                up = scipy.ndimage.gaussian_filter(shift, 6.0)
            ...
            for fld in outM.getFloatFieldList():
                if applyHills:
                    applyHillMap(outM.getFloatField(fld).get_nd_array(), mapH)
                if fld in salt_value:
                    fillModel(
                        slt, outM.getFloatField(fld).get_nd_array(), salt_value[fld]
                    )
            for fld in outM.getIntFieldList():
                if applyHills:
                    applyHillMap(outM.getIntField(fld).get_nd_array(), mapH)
                if fld in salt_value:
                    fillModel(
                        slt, outM.getIntField(fld).get_nd_array(), salt_value[fld]
                    )
                elif fld == "layer":
                    fillModel(slt, outM.getIntField(fld).get_nd_array(), ievent)

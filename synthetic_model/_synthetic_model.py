...
def getGeoField(hyper: Hypercube, storage: str = "dataFloat"):
    return get_sep_vector(hyper, storage=storage)
...
class GeoModel:
    def __init__(self, hyper, *args, **kw):
        ...
        if len(args) > 0:
            ...
            if not isinstance(hyper, Hypercube):
            ...
            self.intFields["layer"].get_nd_array().fill(0)
            self.floatFields = {}
            self.fills = {"layer": -1}
            for fld in basement.keys():
                self.floatFields[fld] = getGeoField(hyper)
                self.floatFields[fld].get_nd_array().fill(basement[fld])
            ...
            self.ax2 = hyper.get_axis(2)
            self.ax3 = hyper.get_axis(3)
            rgt = False
            if "rgt" in kw:
    ...
    def getCreateFloatField(self, param, fill, basement):
        ...
        if param not in self.floatFields:
            self.floatFields[param] = getGeoField(self.getPrimaryProperty().get_hyper())
    ...
    def getCreateIntField(self, param, fill, basement):
        ...
        if param not in self.intFields:
            self.intFields[param] = getGeoField(
                self.getPrimaryProperty().get_hyper(), storage="dataInt"
            )
            self.basements[param] = basement
            self.fills[param] = fill
    ...
    def findMaxDeposit(self):
        ...
        return findMaxD(self.getLayer().get_nd_array())
    ...
    def expand(self, beg: int, end: int):
        ...
        return @geoModel
        """
        axIn = self.getPrimaryProperty().get_hyper().get_axis(1)
        ax1 = Axis(
            n=axIn.n + beg + end, o=axIn.o, d=axIn.d, label=axIn.label, unit=axIn.unit
        )
        hyp = Hypercube(axes=[ax1, self.ax2, self.ax3])
        ...
        for k, v in self.floatFields.items():
            ...
            fill = self.fills[k]
            outA = outM.floatFields[k].get_nd_array()
            inA = self.floatFields[k].get_nd_array()
            outA[:, :, :beg] = self.fills[k]
            outA[:, :, beg : beg + axIn.n] = inA
            outA[:, :, beg + axIn.n :] = self.basements[k]
        for k, v in self.intFields.items():
            outA = outM.intFields[k].get_nd_array()
            base = self.basements[k]
            fill = self.fills[k]
            inA = self.intFields[k].get_nd_array()
            outA[:, :, :beg] = self.fills[k]
            outA[:, :, beg : beg + axIn.n] = inA
            outA[:, :, beg + axIn.n :] = self.basements[k]
        return outM
    ...
    def getGeoFields(self, *args):
        ...
        # primary hyper
        new_hyper = self.get().get_hyperper().clone()

        # add axis to hypercube
        new_hyper.addAxis(Axis(n=nProp, o=0, d=1, label="field"))
        ...
        for field_count, field in enumerate(args):
            ...
            out_sepVec.get_nd_array()[field_count, ...] = self.get(
                param=field
            ).get_nd_array()
    ...
    def writeModel(self, filename, min_clip={}, max_clip={}):
        ...
        for field_count, field in enumerate(self.basement):
            ...
            # clip each field
            out_sepVec.get_nd_array()[field_count, ...] = np.clip(
                out_sepVec.get_nd_array()[field_count, ...],
                a_min=cur_min_clip,
                a_max=cur_max_clip,
            )
    ...
    def writeLayers(self, filename):
        ...
        """
        float_layers_sep = getGeoField(self.getLayer().get_hyperper())
        float_layers_sep.get_nd_array()[:] = self.getLayer().get_nd_array()
        # write to disk

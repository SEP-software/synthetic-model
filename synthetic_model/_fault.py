...
import math
import numba
from synthetic_model._sinc_table import table
import synthetic_model._model_noise as model_noise
import datetime
import numpy as np
...
# "void(int32[:,:,:],int64[:,:],List(float64),boolean[:],float64,float64,float32[:,:],float32[:,:],float32[:,:],int32[:,:,:])",parallel=True)
def shiftIntField(
    fieldIn, ilocs, ds, changed, fill, basement, shift1, shift2, shift3, fieldOut
):
...
def calcAzPerp(
    loc, cylinder_center_axis_2, cylinder_center_axis_3, fault_azimuth, az_perp
):
    if fault_azimuth < 0.0:
        fault_azimuth + 360  # Put azimuth between 0-360
    fault_azimuth *= np.pi / 180.0
    ...
    for ipos in prange(loc.shape[1]):
        ...
        if abs(az_perp[ipos, 0]) < 0.001:
            az_perp[ipos, 0] = 0.0001
...
def calcShifts(
    ...
    ds,
    scalePerp,
    minAxisDiscretization,
    cylinderRotationTheta,
    indicateVal,
    faultHalf,
    thetaDie,
    minToIndicate,
    ...
    for ipos in numba.prange(azPerp.shape[0]):
        ...
        for i1 in range(shiftFieldAxis1.shape[1]):
            ...
            thetaOld = math.atan2(loc_axis_1, azPerp[ipos, 0]) * 180.0 / math.pi

            thetaOld += 360

            thetaCompare = (
                math.atan(loc_axis_1 / azPerp[ipos, 0]) * 180.0 / math.pi + 360.0
            )
            # True radius of current point
            radius = math.sqrt(
                azPerp[ipos, 0] * azPerp[ipos, 0] + loc_axis_1 * loc_axis_1
            )
            ratioAz = abs(noisyRadius[ipos] - radius) / distDie
            ratioTheta = abs(thetaOld - theta0) / thetaDie
            if ratioAz < 1 and ratioTheta < 1:
                scaleAz = math.cos(ratioAz * math.pi / 2.0)
                scaleTheta = math.cos(ratioTheta * math.pi / 2.0)
                scaleTot = (
                    scaleAz * scaleTheta * math.cos(math.pi / 2.0 * scalePerp[ipos])
                )
                ...
                if (
                    abs(radius - noisyRadius[ipos]) / minAxisDiscretization < 0.999
                    and shiftTheta / cylinderRotationTheta > minToIndicate
                ):
                ...
                dPR = radius * math.cos(thetaNew * math.pi / 180.0)
                dpO = radius * math.cos(thetaOld * math.pi / 180.0)
                newZ = (
                    radius * math.sin(thetaNew * math.pi / 180.0) + cylinderCenterAxis1
                )
                ...
                if (
                    abs(shiftFieldAxis1[ipos, i1]) > 0.01
                    or abs(shiftFieldAxis2[ipos, i1]) > 0.01
                    or abs(shiftFieldAxis3[ipos, i1]) > 0.01
                ):
                    changedPos[ipos] = True
...
def shiftFloatField(
    fieldIn, ilocs, ds, changed, fill, basement, shift1, shift2, shift3, syncT, fieldOut
):
    for ipos in numba.prange(shift1.shape[0]):
        i2 = ilocs[1, ipos]
        i3 = ilocs[0, ipos]
        if changed[ipos]:
            for i1 in range(fieldIn.shape[2]):
                if (
                    abs(shift1[ipos, i1]) > 0.01
                    or abs(shift2[ipos, i1]) > 0.01
                    or abs(shift3[ipos, i1]) > 0.01
                ):
                    ...
                    if l1 < -.4999999:
                        ...
                        is1 = max(
                            ...
                            min(
                                syncT.shape[0] - 1,
                                int((l1 - int(l1)) * syncT.shape[0] + 0.5),
                            ),
                        )
                        is2 = max(
                            0,
                            min(
                                syncT.shape[0] - 1,
                                int((l2 - int(l2)) * syncT.shape[0] + 0.5),
                            ),
                        )
                        is3 = max(
                            0,
                            min(
                                syncT.shape[0] - 1,
                                int((l3 - int(l3)) * syncT.shape[0] + 0.5),
                            ),
                        )
                                    ...
                                    v = (
                                        fieldIn[
                                            max(
                                                0,
                                                min(
                                                    fieldOut.shape[0] - 1, il3 - 3 + ic
                                                ),
                                            ),
                                            max(
                                                0,
                                                min(
                                                    fieldOut.shape[1] - 1, il2 - 3 + ib
                                                ),
                                            ),
                                            max(
                                                0,
                                                min(
                                                    fieldOut.shape[2] - 1, il1 - 3 + ia
                                                ),
                                            ),
                                        ]
                                        * syncT[is3, ic]
...
class Fault(Event):
    ...
    def applyBase(
        ...
        indicateF: bool = False,
        dir: int = 1,
        faultZone: float = 0.0,
        indicateValue: float = 0.05,
        indicateMark: int = 1,
        seed=None,
        begx: float = 0.5,
        begy: float = 0.5,
        ...
        dz: float = 0.0,
        daz: float = 0.5,
        deltaTheta: float = 0.1,
        dist_die: float = 0.2,
        theta_die: float = 5.0,
        theta_shift: float = 3.0,
        radius_freq: float = 4.0,
        radiusSkew: float = 0.02,
        **kw
    ) -> GeoModel:
        """
        Fault a model
        ...
        if "extentInLine" in kw:
            dist_die = kw["extentInLine"] / max(
                axes[1].d * axes[1].n, axes[2].d * axes[2].n
            )
        if "extentCrossLine" in kw:
            perp_die = kw["extentCrossLine"] / max(
                axes[1].d * axes[1].n, axes[2].d * axes[2].n
            )
        if "radius" in kw:
            radius = kw["radius"]
            if not "angle" in kw:
                kw["angle"] = 0
            dz = (
                math.sin(0.01745 * kw["angle"])
                * kw["radius"]
                / max(axes[1].d * axes[1].n, axes[2].d * axes[2].n)
            )
            daz = (
                math.cos(0.01745 * kw["angle"])
                * kw["radius"]
                / max(axes[1].d * axes[1].n, axes[2].d * axes[2].n)
            )
        else:
            da = max(axes[1].d * axes[1].n, axes[2].d * axes[2].n) * 0.5
            if "daz" in kw:
        ...
        if "ruptureLength" in kw:
            theta_die = kw["ruptureLength"] / kw["radius"] / math.pi * 180.0
        ...
        if "shift" in kw:
            theta_shift = (kw["shift"] / math.pi / radius / 2.0) * 180.0
        ...
        if cylinderRotationTheta > thetaDie / 2.0:
            raise Exception(
                "A shift more than 1/2 theta_die is unrealistic and unalloawed"
            )

        direction = dir

        theta0 = (math.atan2(dz, daz)) * 180.0 / math.pi
        theta0 += 360
        ...
        cylinderCenterAxis2 = faultBegAxis2 - daz * math.cos(
            faultAzimuth / 180 * math.pi
        )
        cylinderCenterAxis3 = faultBegAxis3 - daz * math.sin(
            faultAzimuth / 180 * math.pi
        )
        ...
        if indicateF:
            indicateFloat = inM.getCreateFloatField("indicator", 0.0, 0.0).get_nd_array()
            fake = 1.0
        else:
            fake = 0.0
        ...
        if indicateI:
            indicateInt = inM.getCreateIntField("indicator", 0, 0).get_nd_array()
        ...
        perpS = azPerp[:, 1] / perpScale + np.random.rand() * max(
            ds[1] * ns[1], ds[2] * ns[2]
        )
        noiseyRadius = radius * (1.0 + radiusSkew * model_noise.fractal(perpS))
        ...
        for fld in outM.getIntFieldList():
            inF = inM.getIntField(fld).get_nd_array()
            outF = outM.getIntField(fld).get_nd_array()
        ...
        for fld in outM.getFloatFieldList():
            inF = inM.getFloatField(fld).get_nd_array()
            outF = outM.getFloatField(fld).get_nd_array()
        ...
        if indicateI:
            ...
            j += tmpI
        if indicateF:
            j = outM.getFloatField("indicator").get_nd_array()
            j += tmpF

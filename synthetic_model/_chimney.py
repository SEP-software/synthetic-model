...
from synthetic_model._simplex_noise as SimplexNoise
import numba
import math
import numpy as np
import random
...
import datetime
import synthetic_model._event as Event
import synthetic_model._structural_smoothing as structural_smoothing
        ...
        for i2 in range(ns[1]):
            ...
            for i1 in range(ns[0]):
                ...
                if z < pockDepth * 3:
                    ...
                    iXNoise = int(fXNoise + 0.5)
                    iYNoise = int(fYNoise + 0.5)
                    ...
                    if dist <= haltWidthAtLocation:  # we are inside
                        ...
                        zscale = 1
                        if z > pockDepth:
                            zscale = 1.0 - (z - pockDepth) / (2 * pockDepth)
                        shift[i3, i2, i1] = (
                            -pockDepth * math.cos(fr * math.pi / 2.0) * zscale / ds[0]
                        )
                    ...
                    halfAtLocation = (
                        (1.0 - fXNoise)
                        * (1.0 - fYNoise)
                        * halfWidthA[iYNoise, iXNoise, i1]
                        + (fXNoise)
                        * (1.0 - fYNoise)
                        * halfWidthA[iYNoise, iXNoise + 1, i1]
                        + (1.0 - fXNoise)
                        * (fYNoise)
                        * halfWidthA[iYNoise + 1, iXNoise, i1]
                        + (fXNoise)
                        * (fYNoise)
                        * halfWidthA[iYNoise + 1, iXNoise + 1, i1]
                    )
...
@numba.njit(parallel=True)
def fix_half_width(widthIn, mean, stdDev, grad, minV):
    """
    ...
    for i3 in numba.prange(fldIn.shape[0]):
        for i2 in range(fldIn.shape[1]):
            ...
            for i1 in range(fldIn.shape[2]):
                ...
                if iloc + i1 < 0 or abs(tmp2[iloc + i1] - fill) < 0.001:
                    fldOut[i3, i2, i1] = fill
                    ...
                    for ifilt in range(8):
                        fldOut[i3, i2, i1] += (
                            sincTab[itab, ifilt] * tmp[iloc + ifilt + i1]
                        )


class Chimney(_event.event):
    ...
    def applyBase(
        ...
        zeroEnd: float = 0.9,
        dampV: float = 0.9,
        minWidth: float = 0.015,
        gradWidth: float = 0.1,
        applySmoothing: bool = False,
        filterLen: int = 3,
        smoothLen: int = 60,
    ) -> GeoModel:
        """
        ...
        """
        axes = modIn.getPrimaryProperty().get_hyper().axes
        ...
        halfWidth = width * max(exts[1], exts[2]) / 2.0
        ...
        if iZeroBeg < 0:
            raise Exception("Illegal zeroBeg must be greater than 0")
        if iFullBeg < iZeroBeg:
            raise Exception("Illegal chimnet description iFullBeg<iZeroBeg")
        if iFullEnd < iFullBeg:
            raise Exception("Illegal chiment description iFullEnd<iFullBeg")
        if iZeroEnd < iFullEnd or iZeroEnd >= axes[0].n:
            raise Exception(
                "Invalid chimney descrption zeroEnd < fullEnd or zeroEnd>=1."
            )
        ...
        noiseCenter = simplex_noise.simplexNoise(random.randint(0, 1000 * 1000 * 1000))
        noiseEdge = simplex_noise.simplexNoise(random.randint(0, 1000 * 1000 * 1000))

        pi = math.atan(1.0) * 2
        ang1L = np.linspace(-pi, pi + 0.1, 185, dtype=np.float32)
        ang2L = np.linspace(-pi, pi + 0.1, 185, dtype=np.float32)
        zL = np.linspace(0.0, 9.0, ns[0], dtype=np.float32)

        centerX = (
            noiseCenter.noise2D(zL, zL[:10], amplitude=devCenter / 1.875, n_octaves=4)[
                0, :
            ]
            + center2
        )
        ...
        halfWidthA = halfWidth + noiseEdge.noise3D(
            zL, ang2L, ang1L, amplitude=devEdge / 1.875
        )

        halfWidthA = fix_half_width(halfWidthA, halfWidth, devEdge, gradWidth, minWidth)
        del ang1L
        del ang2L

        shift, dampT = calc_shift_damp(
            iZeroBeg,
            iFullBeg,
            iFullEnd,
            iZeroEnd,
        ...
        del centerY
        del halfWidthA
        modIn.getCreateFloatField("damp", 1.0, 1.0)
        ...
        for fld in modOut.getFloatFieldList():
            ...
            applyShiftFloat(
                modIn.getFloatField(fld).get_nd_array(),
                modOut.getFloatField(fld).get_nd_array(),
                shift,
                sinc,
                fill,
            )
        for fld in modOut.getIntFieldList():
            ...
            applyShiftInt(
                modIn.getIntField(fld).get_nd_array(),
                modOut.getIntField(fld).get_nd_array(),
                shift,
                fill,
            )
        ...
        if applySmoothing:
            ...
            vS = modOut.getPrimaryProperty().get_nd_array()[b3:e3, b2:e2, :]
            dS = dampT[b3:e3, b2:e2, :]
            d2 = structural_smoothing.smoother(vS, mod=dS, hw=filterLen, hw2=smoothLen)
            dampT[b3:e3, b2:e2, :] = d2
            print(b2, e2, b3, e3)
            multDamp(modOut.getFloatField("damp").get_nd_array(), dampT)
        else:
            multDamp(modOut.getFloatField("damp").get_nd_array(), dampT)
        return modOut
...
class Big(Chimney):
    ...
    def __init__(
        ...
        if depth < 0.02 and depth > 0.7:
            print("Depth must be between .02 and .7")

        # Assume the basement takes .15
        len = 0.82 - depth
        kws["zeroBeg"] = depth
        kws["fullBeg"] = depth + 0.1 * len
        kws["fullEnd"] = depth + 0.8 * len
        kws["zeroEnd"] = 0.82
        del kws["self"]
        del kws["depth"]
        del kws["__class__"]
        kws["minWidth"] = width * 0.76
        kws["filterLen"] = 3
        kws["smoothLen"] = 60
        super().__init__(**kws)
